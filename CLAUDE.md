...
- Orient once per unfamiliar area: read only `## God Nodes` and
  ...
  … and carries no signal. See "Community labeling" below.
...
**Community labeling:** community names are LLM-generated, and no backend is currently configured
(no `GEMINI_API_KEY` / `GOOGLE_API_KEY` / `ANTHROPIC_API_KEY` / `OPENAI_API_KEY` in the environment),
so communities are placeholders. Everything else in the report is AST-derived and always accurate.
To enable: set a key, then `graphify label .` — after that, `## Community Hubs (Navigation)` becomes
the best single orientation section and should be added to the orientation read above.
